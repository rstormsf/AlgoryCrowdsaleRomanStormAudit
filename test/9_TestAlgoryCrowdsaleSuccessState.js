...
contract('Test Algory Crowdsale Success State', function(accounts) {
    it("prepare suite by assign deployed contracts and set dates to make success state", function () {
    ...
    it("should be in success state", function () {
        return algoryCrowdsale.getState()
            .then(function (state) {
                assert.equal(state.toNumber(), 4, 'Crowdsale state is not in success state');
            })
    });
    it("shouldn't be finalized yet", function () {
        return algoryCrowdsale.finalized()
            .then(function (isFinalized) {
                assert.equal(isFinalized, false, 'Crowdsale is finalized');
            })
    });
    it("shouldn't allow to transfer tokens by anyone in success sate", function () {
        let error;
    ...
    it("should finalized crowdsale and release token transfer", function () {
        return algoryCrowdsale.finalize()
        .then(function () {
            return algoryCrowdsale.finalized()
        })
        .then(function (isFinalized) {
            assert.equal(isFinalized, true, 'Crowdsale is not finalized');
            return algoryToken.released()
        })
        .then(function (isReleased) {
            assert.equal(isReleased, true, 'Algory token is not released');
        })
    });
    it("should be in finalized state", function () {
        return algoryCrowdsale.getState()
            .then(function (state) {
                assert.equal(state.toNumber(), 6, 'Crowdsale state is not in finalized state');
            })
    });
    it("should able to transfer tokens by everyone who is owner", function () {
        let initBalance = bigNumber(0);
        let tokensToTransfer = bigNumber(7777);
        return algoryToken.balanceOf(accounts[12]).then(function (balance) {
            initBalance = balance;
            assert.ok(balance.greaterThanOrEqualTo(tokensToTransfer), 'Sender has not enough ALG')
        })
        .then(function () {
            return algoryToken.transfer(accounts[11], tokensToTransfer, {from: accounts[12]})
        })
        .then(function () {
            return algoryToken.balanceOf(accounts[11]).then(function (balance) {
                assert.deepEqual(balance, initBalance.plus(tokensToTransfer), 'Balance is invalid after transfer')
            })
        })
    });
});
